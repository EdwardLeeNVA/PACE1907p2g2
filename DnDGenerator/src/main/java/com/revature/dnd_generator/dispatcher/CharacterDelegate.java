...
public class CharacterDelegate {
	...
	public static Object processer(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		ObjectMapper mapper = new ObjectMapper();
		//get path length
		String[] path = req.getRequestURI().split("/");
		//get all from current user
		if(path.length == 4) {
			LOGGER.info("Fetching all with userid: " +req.getSession().getAttribute("playerID").getClass());
